...
def get_warped_alpha_b_t(
    ...
    s_b: torch.Tensor,                   
    target_log_alpha_at_0: torch.Tensor  
    ) -> torch.Tensor:
    
    L_base_t = torch.logit(base_alpha_bar_t) 
    
    # s_b could be (B,N_Blk,1) or (B,N_Blk)
    # base_log_alpha_bar_at_0 is scalar
    # target_log_alpha_at_0 is scalar
    # L_base_t is (B,N_Blk) or (B,N_Blk,N_t)
    # Unsqueeze s_b if it's (B,N_Blk) and L_base_t is (B,N_Blk,N_t)
    if s_b.ndim == 2 and L_base_t.ndim == 3: # s_b is (B,N_Blk), L_base_t is (B,N_Blk,N_t)
        s_b_expanded = s_b.unsqueeze(-1)
    else:
        s_b_expanded = s_b # Assume shapes are compatible or s_b is (B,N_Blk,1)

    A_b_t = s_b_expanded * (L_base_t - base_log_alpha_bar_at_0) + \
            target_log_alpha_at_0
