...
class TestNewFeatures(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.algo_config_dict = {
            ...
            'clip_search_delta': 0.1, # Added default
            'clip_search_widths': [0.5], # Added default
            'fix_clipping': False, # Added default
        }
        ...
        cls.sampling_config_dict = { 
            ...
            'nucleus_p': 1.0, 
            'var_length': False,
            'logdir': './sample_logs/test_samples.csv', 
        ...
        cls.diffusion_config = OmegaConf.create({
            ...
            'data': {'tokenizer_name_or_path': 'mock', 'wrap': True}, # Added wrap for metrics init
            'loader': {'eval_batch_size': 1}, # Added for metrics init
            'noise': {
                'type': 'cosine',
                'eps': 1e-3 
            }, 
    ...
    def test_diffusion_q_xt_and_sample_t(self):
        ...
        B_stat_test, N_Tokens_stat_test = 8, 256 
        block_size_stat_test = self.diffusion_config.block_size 
        # N_Blk_stat_test = N_Tokens_stat_test // block_size_stat_test # Not used directly here
        
        model_stat, config_stat = self._create_mock_diffusion_model({
            'model': {'length': N_Tokens_stat_test}, 
            'block_size': block_size_stat_test,
            'training': {'resample': False} 
        })
        
        x0_stat = torch.randint(0, config_stat.vocab_size, (B_stat_test, N_Tokens_stat_test), device=self.device)
        
        p_region1_stat = torch.full((B_stat_test, N_Tokens_stat_test // 2), 0.1, device=self.device)
        p_region2_stat = torch.full((B_stat_test, N_Tokens_stat_test // 2), 0.9, device=self.device)
        p_distinct_stat = torch.cat([p_region1_stat, p_region2_stat], dim=1)
        
        xt_distinct_stat = model_stat.q_xt(x0_stat, p_distinct_stat.unsqueeze(-1))
        
        masks_region1_stat = (xt_distinct_stat[:, :N_Tokens_stat_test//2] == model_stat.mask_index).float().mean()
        masks_region2_stat = (xt_distinct_stat[:, N_Tokens_stat_test//2:] == model_stat.mask_index).float().mean()
        
        self.assertAlmostEqual(masks_region1_stat.item(), 0.1, delta=0.05, msg=f"Region 1 (p=0.1) mask rate: {masks_region1_stat.item()}")
        self.assertAlmostEqual(masks_region2_stat.item(), 0.9, delta=0.05, msg=f"Region 2 (p=0.9) mask rate: {masks_region2_stat.item()}")
